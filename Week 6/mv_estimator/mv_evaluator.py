""" mv_evaluator.py - Week 6 Day 3/4 baseline evaluator, adapted for football.

Mirrors the threaded, Item-object-based Tester (datapoint.price/.title ->
datapoint.value/.name here) used for baseline/traditional-ML/frontier
zero-shot evaluation - this runs BEFORE tokenization, directly against
PlayerItem test objects.

This is a DIFFERENT evaluator from the Week 7 fine-tuned-model tester
(which works on tokenized prompt/completion dict rows) - don't confuse the
two; this one is for the pre-fine-tuning baseline comparisons.
"""
...
class Tester:
    ...
    def color_for(self, error, truth):
        # Transfer values span orders of magnitude (10k to 150M+ euros), so
        # lean on relative error rather than flat euro thresholds - a $40/$80
        # style absolute cutoff makes no sense at this scale.
        rel = error / truth if truth else 1.0
        if rel < 0.15:
            return "green"
        elif rel < 0.35:
            return "orange"
    ...
    def chart(self, title):
        ...
        fig = px.scatter(
            ...
            title=title,
            labels={"truth": "Actual Market Value (\u20ac)", "guess": "Predicted Market Value (\u20ac)"},
            width=1000,
            height=800,
        )
    ...
    def report(self):
        ...
        title = (
            f"{self.title} results<br><b>Error:</b> \u20ac{average_error:,.0f} "
